...
\begin{abstract}
移动智能终端和移动互联网的发展已经使人们的日常生活发生了极大的变革。之前需要在多个设备上才能完成的工作，如即时通信、浏览网页、拍照、购物、支付等，现在全都可以通过以智能手机为代表的智能手持设备完成。而这种功能的集成化使得仅在一台智能手机中就产生和存储了大量的用户隐私信息。

在多种智能操作系统中，鸿蒙系统的出现标志着我国技术的飞速发展。鸿蒙基于微内核的面向全场景的分布式操作系统，同时满足全场景流畅体验、架构级可信安全、跨终端无缝协同以及一次开发多端部署的要求。然而由于鸿蒙系统刚刚出现，它的很多模块并不完善，特别是安全模块，可以说是一个空白的区域，因此恶意应用可以窃取用户的敏感信息、使用用户的隐私设备、将用户的敏感信息泄露给第三方。\par
对于上述隐私威胁，目前国内并没有针对性的研究，对于鸿蒙内核的架构也没有非常完善的分析。\par
本作品首先立足于OpenHarmony系统，搭建了一个以ArkUI为前端界面、以NAPI为接口的系统级应用监测框架；之后在该框架中基于Libbpf技术，构建了一个能够与LiteOS内核进行交互的具有高拓展性与可移植性的Hook模型，从而实现对硬件设备、网络流量、数据信息等多种应用隐私行为的实时监测；同时，本作品凭借Sidercar模式与Libbpf结合后的优越性能，在客户端与服务端之间传递获取到的轻量级数据，并利用数据在服务端用改进后的FNN模型对应用行为进行分析，最终构建了一个完善的弹性防护机制，能够根据用户的自定义选择对应用进行全方位、自适应的安全性管控，能够在多内核版本、多平台运行。

使用ebpf技术进行系统监测有以下优势：

\begin{enumerate}[(1)]
\item 速度和性能。 eBPF 可以将数据包处理这项工作从内核空间转移到用户空间。同时，eBPF 还支持即时 (JIT) 编译器。在字节码被（JIT）编译完成后，会直接调用 eBPF，而不是对每个方法的字节码进行新的解释。

\item 低侵入性。 当作为调试器时，eBPF 不需要停止程序来观察其状态。

\item 安全。 程序被有效地沙箱化了，这意味着内核源代码仍然受到保护并保持不变。eBPF程序的验证步骤确保资源不会被运行无限循环的程序阻塞。

\item 方便。 创建hook内核函数的代码比构建和维护内核模块的工作要少。

\item 可编程性。 使用 eBPF 有助于在不添加额外层的情况下增加环境的功能丰富性。由于代码直接在内核中运行，因此可以在 eBPF 事件之间存储数据，而不是像其他跟踪器那样转储数据。

\item 统一追踪。 eBPF 为我们提供了一个单一、强大且易于访问的流程跟踪框架，这增加了可见性和安全性。
\end{enumerate}

本作品的创新性主要体现在以下方面：

\begin{enumerate}[(1)]
\item 本作品是建立于鸿蒙OS——OpenHarmony上的，该系统为国产自研Linux内核系统，近几年才兴起火热。对于该系统的底层的隐私监测系统，此前未有研究人员在此方向尝试，尽管其它系统平台有不少成熟的应用，但都不能简单地移植过来，不具备兼容性与可行性。

\item 本作品使用的libbpf相较于传统的BCC有着非常突出的优势，包括内存占用少、编译更加方便快速，实现了轻量级的目标。

\item 本作品的应用框架具有可拓展性，其不仅仅局限于我们当前实现了的隐私行为监测，之后还可以进行拓展扩充，通过在已有框架中添加模块的方式，监测其它诸如应用安装、后台权限等等更多的应用程序行为，从而实现更全面的安全性覆盖。

\end{enumerate}\par
本作品的实用性主要体现在以下方面：

\begin{enumerate}[(1)]
\item 本作品具有良好的可移植性，构造了一个通用的隐私监测模型。可以用于多种以Linux作为内核的系统的底层框架，在上层只需要做一定的适配工作即可轻松监控对系统中各种硬件设备的使用情况。

\item 本作品在保障用户隐私的同时占用较少的资源。在底层实现中，依靠对系统指令级别的插桩点进行监控，比起在应用层的监控，其占用资源更少；在前端开发中，使用了基于拓展TS的声明式开发范式，无需JS Framework进行页面DOM管理，渲染更新链路更为精简，占用内存更少。

\item 监测效果好，可为用户提供可靠的隐私保护。本作品的核心监测工作在内核层实现，因此可以保证较高的拦截率，监测效果好，可为用户提供可靠的隐私保护。

\item 可拓展性好，有丰富的应用场景和广阔的应用前景。本作品除了可以用于监控OpenHarmony系统中的设备，同时还可以结合OpenHarmony万物互联的特性，监控对同一网络下其他设备的使用。
\end{enumerate}\par
本次作品设计实现了一个名为“透镜”的内核级系统隐私行为监测与防护应用。“透镜”能够帮助用户监测并控制其他应用对系统进行隐私行为的调用。性能测试表明，“透镜”可以实现精准的监测防护，并且带来的性能开销在用户可接受的范围内。\par
\textbf{Keywords: }OpenHarmony，eBPF，系统检测
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\columnwidth, height=0.5\textwidth]{./pic/12}
\caption{\small{鸿蒙系统特性}}
\end{figure}
...
OpenHarmony目前只具有基础的一些能力，但在隐私保护等方面还有所欠缺，且由于操作系统过新，目前相关领域工作较少。用户在上网过程中，身份信息泄露的途径有很多，最为常见的就是手机中的APP在后台擅自调用敏感权限来获取我们的个人信息。其中危害最大的莫过于软件偷偷打开摄像头对用户进行拍照或打开录音机对用户进行监听等。对此OpenHarmony还未在安全子系统中做任何的安全监控处理。
...
当其它应用发生隐私行为时，透镜会根据用户的设置显示系统通知告知用户该隐私行为的发生。
        \pic_normal{0.5}{./pic/218}{后台通知}
\end{enumerate}
\subsubsection{系统开发环境}
操作系统：Linux 20.04、OpenHarmony 3.1\par
开发工具：DevEco Studio 3.0.0.9、gcc 11.0、Vim、cmake、Qt 5.15.2\par
开发语言：C、eTS
\subsection{系统架构}
...
NAPI通过注册函数进行模块的注册，其接受一个全局变量参数，全局变量结构体中定义了模块名及模块初始化函数。在模块的初始化中，我们可以定义模块需要暴露的方法及属性。
  ...
  \item NAPI实现
        ...
        区别如下图所示：
        \pic_normal{0.8}{./pic/40}{NAPI的不同接口区别}
        NAPI有关的接口信息可以在Node.js官方文档中查看，在这里我们主要使用C接口在实现过程中主要使用的接口如下

\end{enumerate}

\subsection{基于Libbpf的内核级隐私监测hook模型}
...
运行 eBPF 程序的步骤：\par
1. 用户空间将字节码和程序类型一起发送到内核，程序类型决定了可以访问\par
的内核区域(主要是 BPF 辅助函数的各种子集)。\par
2. 内核在字节码上运行验证器，以确保程序可以安全运行(kernel/bpf/veri-\par
fier.c)。\par
3. 内核将字节码编译为本地代码，并将其插入(或附加到)指定的代码位\par
置。(如果启用了 JIT 功能，字节码编译为本地代码)。\par
4. 插入的代码将数据写入环形缓冲区或通用键值 map。\par
5. 用户空间从共享 map 或环形缓冲区中读取结果值。\par
...
这一部分是我们底层应用架构的核心，涉及系统中框架层与内核层的交互。实现主要依托于Libbpf，分为.c和.bpf.c两个文件，分别用于\important{启动检测}、与内核进行交互\important{获取内核中得到的数据}，以及对得到的数据进行\important{过滤、去重、统计、分类} \important{、函数封装}等处理。最后这些处理好的数据与函数还会再向上传递，以供上层NAPI以及前端的使用。
...
\paragraph{数据处理}
这部分首先要设置strict模式，之后加载并检验BPF程序，添加tracepoint插桩点，设置ring缓冲区.\par
...
\section{作品创新性与实用性}
\subsection{创新性}
本作品的创新性主要包括以下几个部分，分别是\important{系统特性、内核交互方式、}\important{框架与模块属性优势}。


\begin{enumerate}

\item 系统特性方面，\textbf{“透镜”是一款主要面向鸿蒙系统的安全应用，这是目前国内研究领域的空白。}我们的作品是建立于鸿蒙OS——OpenHarmony上的，该系统为国产自研Linux内核系统，近几年才兴起火热。其系统新，故而也存在很多问题，比如资料匮乏、文件不完善、接口少、框架应用不足等。尤其是位于底层的隐私监测系统，此前未有研究人员在此方向尝试；而且尽管其它系统平台有不少成熟的应用，但都不能简单地移植过来，不具备兼容性与可行性。也正因如此，我们的作品应运而生，在一定程度上填补了鸿蒙系统这部分的空白，在未来也可以继续进行完善和补充，为我国鸿蒙系统的不断发展做出贡献。

\item 内核交互方式上，\textbf{“透镜”将内核的数据类型构建在内核，bpf程序只要依赖这个头文件就行，不需要安装内核头文件的包，实现了性能的提升和轻量级的目标}。我们所采用的是基于Libbpf的应用框架，libbpf相较于传统的BCC有着非常突出的优势，包括内存占用少、编译更加方便快速等等。我们通过它，可以在内核中监测到大量的系统调用行为，并通过一系列数据处理最后获得我们想要监测的使用情况、隐私数据，上传给前端。

\item 我们的框架与属性优势主要体现在\textbf{“透镜”具有可拓展性和可移植性，这使得我们的应用框架中除了隐私行为监测模块还可以根据用户其他需求添加其他的系统监测功能，并且在多内核版本，多操作系统平台进行安装}。第一点我们的系统应用检测框架具有\textbf{可拓展性}，其不仅仅局限于我们当前实现了的隐私行为监测，之后还可以进行拓展扩充，通过在已有框架中添加模块的方式，监测其它诸如应用安装、后台权限等等更多的应用程序行为，从而实现更全面的安全性覆盖。第二点是我们的系统架构具有\textbf{可移植性}，这也是Libbpf的CO-RE理念核心所在，其编译一次便可以运行于任何地方，经过我们的测试，只需要简单修改上层接口，便可以很好地运行于其它系统，比如Linux与Android。
